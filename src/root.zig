...
pub const Opcode = @import("code.zig").Opcode;
pub const Operation = @import("code.zig").Operation;
...
pub const Interpreter = struct {
    ...
    fn callIndex(self: *Interpreter, index: usize, params: []Value) CallError!Stack {
        ...
        if (index < import_count) {
            // call imported function instead of defined function
            // return self.callImport(index, params);
            return .{ .gpa = gpa, .array_list = .empty };
        }

        const local_index = index - import_count;

        // std.log.info("call index: {d}, local index: {d}, params: {any}", .{ index, local_index, params });

        const body = self.module.code.functions[local_index];
    ...
    pub fn execute(self: *Interpreter, frame: *Frame) CallError!void {
        for (frame.code) |operation| {
            ...
            //         std.log.warn("call {s}", .{if (i < exports.len) exports[i].name else "unknown"});
            //         _ = try callIndex(self, i, &.{});
            //     },
            //     else => {},
            // }
    ...
    pub fn operate(self: *Interpreter, op: Operation, frame: *Frame) CallError!void {
        switch (op) {
            ...
            .call => |i| {
                ...
                frame.stack.array_list.items.len -= function_type.params.len;
